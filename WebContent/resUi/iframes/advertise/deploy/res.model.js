...
(function() {
    ...
    res.model.init = function() {
        ...
        res.ui.root.model.active.resource = "advertise";
    ...
    res.model.update = function(event, data) {
        ...
        switch (event) {
        case "deploy.advertise.getDeployStoreAndGroup.successful":
        case "deploy.advertise.getDeployStoreAndGroup.failed":
            ...
            if (model.doRefreshSchedule) {
                res.ui.send({
                    ...
                    event : "deploy.advertise.getSchedule",
                    data : {
                        resource : "advertise"
                    }
                });
            ...
            break;
        case "deploy.advertise.getSchedule.successful":
        case "deploy.advertise.getSchedule.failed":
...
//          break;
        case "deploy.advertise.setSchedule.successful":
            if (model.message == "SaveInvoke") {
                window.parent.res.ui.root.model.popup = "Wait";
                res.ui.send({
                    context : res.ui.context,
                    event : "deploy.advertise.invokeTransfer",
                    data : {
            ...
            break;
        case "deploy.advertise.setSchedule.failed":
            model.message = "Save.Failed";
            res.ui.root.dialog = "feedback";
            break;
        case "deploy.advertise.getDeployStatus.successful":
            model.deployLogsBtn = false;
            window.parent.res.ui.root.model.popup = "";
            model.deployStatus = data;
            model.update();
            break;
        case "deploy.advertise.getDeployStatus.failed":
            model.deployLogsBtn = false;
            window.parent.res.ui.root.model.popup = "";
            break;
        case "deploy.advertise.getDeployDetail.successful":
            ...
            break;
        case "deploy.advertise.getDeployDetail.failed":
            model.deployLogsBtn = false;
            break;
        case "deploy.advertise.saveDeployDetail.successful":
        case "deploy.advertise.saveDeployDetail.failed":
            ...
            break;
        case "deploy.advertise.viewTableStore.failed":
        case "deploy.advertise.viewTableStore.successful":
            ...
            break;
        case "deploy.advertise.editTableStore.failed":
        case "deploy.advertise.editTableStore.successful":
            ...
            case "success":
                ...
                res.ui.send({
                    context : res.ui.context,
                    event: "deploy.advertise.getDeployStoreAndGroup",
            ...
            break;
        case "deploy.advertise.viewTableGroup.successful":
        case "deploy.advertise.viewTableGroup.failed":
            model.storesGroupCsv = data.storeEntries;
            break;
        case "deploy.advertise.editTableGroup.failed":
        case "deploy.advertise.editTableGroup.successful":
            model.resultCode = data.status;
            ...
            res.ui.root.popupMessage = "popupMessage";
            break;
        case "deploy.advertise.getSysFileTree.successful":
        case "deploy.advertise.getSysFileTree.failed":
...
//      this.ID = "";
    ...
    Config = function(resource) {
        this.resource = resource; // example: "pickList", "notices", "options", "usability","advertise"
            ...
            if (this.schedule && this.schedule.deploy) {
                for (var i = 0; i < this.schedule.deploy.length; i++) {
                    if (this.schedule.deploy[i].company.id == company) {
                        this.schedule.deploy[i].company.name = "SAMPLE";
                        break;
                    }
                }
...
//          }
        ...
        resetPickListEffective : function() {
            if (!this.resources["advertise"]["effective"]) return;
            if (this.resources["advertise"]["effective"].length <= 1) return;

            this.resources["advertise"]["effective"].sort(function(task1, task2) {
                return (new Date(task2.task.effective)) - (new Date(task1.task.effective));
            });

            var effectiveSample = this.resources["advertise"]["effective"];
            var indexTask;
            ...
            var effectiveFolder = this.resources["advertise"]["effective"];
            var hash = {};
            var type = "";
            ...
            for (var i = 0; i < effectiveFolder.length; i++) {
               ...
               /* if (indexTask.target.group) {
                ...
                if (hash[type] !== 1) {
                    ...
                    this.resources["advertise"]["obsolete"].push(effectiveFolder[i]);
                    this.resources["advertise"]["effective"].splice(i, 1);
                    --i;
                }
            }
        },
