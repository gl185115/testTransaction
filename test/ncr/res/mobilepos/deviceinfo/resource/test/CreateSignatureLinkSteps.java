...
public class CreateSignatureLinkSteps extends Steps {
    ...
    @Given("an empty PRM_SIGNATURE_LINK database table")
    public final void anEmptyPRM_SIGNATURE_LINKDatabaseTable(){
        dbInit = new DBInitiator("prm_queuebuster", 
                "test/ncr/res/mobilepos/"
                + "deviceinfo/datasets/PRM_SIGNATURE_LINK_EMPTY.xml", DATABASE.RESMaster);
    }
    ...
    public final void createLink(String storeid, String linkid, String linkinfo) {
        ...
        actualResultBase = deviceInfoTest.createLink("signature", storeid, linkid, linkinfo);
    }
    
    @When("I create a link with invalid type")
    public final void createInvalidLink() {
        actualResultBase = deviceInfoTest.createLink("signature1", "000031", "0", "{}");
    }
    
    @Then("the PRM_SIGNATURE_LINK database table should have the following row(s): $deviceTables")
    public final void thePRM_SIGNATURE_LINKDatabaseTableShouldHaveTheFollowingRow(ExamplesTable expectedDeviceInfoTable) {
        try {
            ...
            for(Map<String, String> expectedDeviceInfo : expectedDeviceInfoTable.getRows()) {
                Assert.assertEquals("Compare the STOREID in PRM_SIGNATURE_LINK row" + i,
                        ...
                        ((String)actualDeviceInfoTable.getValue(i, "storeid")).trim());
