...
public class CardInfoResource {
    ...
    public final CardInfo getCardClassInfo(
        ...
        try {
        	...
        	LOGGER.logAlert(PROG_NAME, functionName, loggerErrorCode,
                    "Failed to get card class info for companyId#" + companyId + ", "
    ...
    @GET
    @Produces({ MediaType.APPLICATION_JSON })
    public final CardTypeInfoResponse getCardTypeInfo(
            @QueryParam("companyId") final String companyId,
            @QueryParam("storeId") final String storeId,
            @QueryParam("cardTypeNo") final String cardTypeNo) {
        String functionName = DebugLogger.getCurrentMethodName();
        tp.methodEnter(functionName)
            .println("companyId", companyId)
            .println("storeId", storeId)
            .println("cardTypeNo", cardTypeNo);
        ...
        try {
            ...
            LOGGER.logAlert(PROG_NAME, functionName, loggerErrorCode,
                    "Failed to get card class type for companyId#" + companyId + ", "
                    + "storeId#" + storeId + " and cardType#" + cardTypeNo + ": "
                    + e.getMessage());
        } finally {
            tp.methodExit(response.toString());
        }
