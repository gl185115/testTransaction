...
public class FormatReceiptByXML extends Formatter {
     ...
     */
    private static final String PROG_NAME = "RcptFmt";

    public FormatReceiptByXML() {
        DebugLogger.getDbgPrinter(Thread.currentThread().getId(),
                getClass());
    }

    public FormatReceiptByXML(String path) {
        DebugLogger.getDbgPrinter(Thread.currentThread().getId(),
                getClass());
        this.xmlPath = path;
    }

    public FormatReceiptByXML(ReceiptMode receipt, String xmlPath)
    ...
    private void format() throws IllegalArgumentException, SecurityException,
        ...
        try {
            ...
            String type = this.receipt.getReceiptType();
            Element root = doc.getRootElement();

            if (root.attributeValue("type").equals(type)) {
                this.lineMaxLimit = StringUtility.convNullToZero(root.element(
                        "ReceiptPaperWidth").getStringValue());

                String language = this.receipt.getLanguage();
                ...
                this.resolveLines(root.element("Form"));
            }
    ...
    private void lineItems(final Element line) throws IllegalArgumentException, SecurityException, IllegalAccessException, InvocationTargetException, NoSuchMethodException, UnsupportedEncodingException, ParseException {
        ...
        for (ItemMode itemMode : itemList) {
            itemMode.setLanguage(this.receipt.getLanguage());
    ...
    private void resolveLines(Element lines) throws IllegalArgumentException,
        ...
        for (Iterator<?> it = lines.elementIterator(); it.hasNext();) {
            ...
            case Barcode:
                ...
                this.addBarcode(line, this.receipt);
                break;
        ...
        if (!StringUtility.isNullOrEmpty(sb.toString())) {
            ...
            if ("true".equalsIgnoreCase(remarksColumnFlg)) {
                if (remarksColumnAlign == 2) {
                    if ("en".equals(this.receipt.getLanguage())) {
...
//                int compareValue = Integer.valueOf(fixedLength) -
//                        invokeMethodStr.length();
//                sb.append(this.appendZero(compareValue, invokeMethodStr));
//            }
//        }
        if (!StringUtility.isNullOrEmpty(property)) {
...
//            sb.append(this.getCurrencySymbol(price));
//        } else if(fixedLength == null){
//            sb.append(this.getAmountByMode(price, minus));
//        }else{
//            String formatStr = this.appendZero(Integer.valueOf(fixedLength));
//            String invokeMethodStr = this.invokeMethod(property, obj);
//            DecimalFormat decimalFormat = new DecimalFormat(formatStr);
//            sb.append(decimalFormat.format(Double.valueOf(invokeMethodStr)));
//        }
        String doPrice = "";
        if(unitFlg != null && Boolean.valueOf(unitFlg)){
//            sb.append(this.getCurrencySymbol(price));
        	doPrice = this.getCurrencySymbol(price);
        } else {
//            sb.append(this.getAmountByMode(price, minus));
        	doPrice = this.getAmountByMode(price, minus);
        }
        if (!StringUtility.isNullOrEmpty(fixedLength)
                && !StringUtility.isNullOrEmpty(appendSign)) {
            sb.append(this.appendSignForString(doPrice, appendSign,
                    appendWay, Integer.valueOf(fixedLength)));
        }else{
        	sb.append(doPrice);
        }
     ...
     *
     */
//    private String appendZero(int size){
...
//    }
     ...
     *
     */
//    private String appendZero(int compareValue,String appendString){
//        int pointIndex = appendString.indexOf(".");
//        if(pointIndex > 0){
//            appendString = appendString.substring(0, pointIndex);
//        }
//        if(compareValue > 0){
//            StringBuilder builder = new StringBuilder(appendString);
//            for(int i = 0 ; i < compareValue ; i ++){
//                builder.insert(0, "0");
//            }
//            return builder.toString();
//        }else if(compareValue < 0){
//            return appendString.substring(Math.abs(compareValue));
//        }else{
//            return appendString;
//        }
//    }
