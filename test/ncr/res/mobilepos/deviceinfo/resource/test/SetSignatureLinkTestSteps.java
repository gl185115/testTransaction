...
public class SetSignatureLinkTestSteps extends Steps {
    ...
    @Given("entries in PRM_SIGNATURE_LINK database table")
    public final void entriesSignatureLinkDB() throws Exception{
        dbInit =  new DBInitiator("Mst_DeviceInfo",
                "test/ncr/res/mobilepos/deviceinfo/"
                + "datasets/PRM_SIGNATURE_LINK_setLink.xml", DATABASE.RESMaster);
    }
    ...
    @When ("I set signature link of storeid{$storeid} and deviceid{$deviceid} to {$signaturelink}")
    public final void setSignatureLink(final String storeid,
            final String deviceid, String signaturelink)
    {
        if(pdc == null){
            pdc = new DeviceInfoResource();
        }
        
        if (signaturelink.equals("null")) {
            signaturelink = null;
        }
        
        resBase = pdc.setSignatureLink(
                storeid, deviceid, signaturelink);
    }
    ...
    public final void theMST_DEVICEINFODatabaseTableShouldHaveTheFollowingRow(ExamplesTable expectedDeviceInfoTable) {
        try {
            ...
            for(Map<String, String> expectedDeviceInfo : expectedDeviceInfoTable.getRows()) {
                ...
                String link = (String)actualDeviceInfoTable.getValue(i, "linksignature");
                if (link == null) {
                    link = "";
                }
                Assert.assertEquals("Compare the linksignature in MST_DEVICEINFO row" + i,
                        expectedDeviceInfo.get("linksignature") == null ? "" : expectedDeviceInfo.get("linksignature"), link);
                i++;
            }
        } catch (Exception ex) {
            ex.printStackTrace();
