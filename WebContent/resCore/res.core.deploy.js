...
(function(){
    ...
    res.core.deploy.onmessage =  function(message) {
        ...
        case "deploy.advertise.getDeployStoreAndGroup":
            res.service.deploy.getDeployStoreAndGroup(message.data,{
                success:function(result){
                    res.core.deploy.send({context:context, event:"deploy.advertise.getDeployStoreAndGroup.successful", data:result});
                },
                error:function(result){
                    res.core.deploy.send({context:context, event:"deploy.advertise.getDeployStoreAndGroup.failed", data:result});
                }
            });
            break;
        case "deploy.advertise.getSchedule":
            res.service.deploy.getSchedule(message.data, {
                success:function(result) {
                    res.core.deploy.send({context:context, event:"deploy.advertise.getSchedule.successful", data: eval(result) });
                },
                error:function(result){
                    res.core.deploy.send({context:context, event:"deploy.advertise.getSchedule.failed", data:result});
                }
            });
            break;
        case "deploy.advertise.setSchedule":
            res.service.deploy.setSchedule(message.data, {
                success:function(result) {
                    res.core.deploy.send({context:context, event:"deploy.advertise.setSchedule.successful", data: undefined});
                },
                error:function(result) {
                    res.core.deploy.send({context:context, event:"deploy.advertise.setSchedule.failed", data: undefined});
                }
            });
            break;
        case "deploy.advertise.getDeployStatus":
            res.service.deploy.getDeployStatus(message.data, {
                success:function(result) {
                    res.core.deploy.send({context:context, event:"deploy.advertise.getDeployStatus.successful", data:result});
                },
                error:function(result) {
                    res.core.deploy.send({context:context, event:"deploy.advertise.getDeployStatus.failed", data:result});
                }
                });
            break;
        case "deploy.advertise.getDeployDetail":
            res.service.deploy.getDeployDetail(message.data, {
                success:function(result) {
                    res.core.deploy.send({context:context, event:"deploy.advertise.getDeployDetail.successful", data:result});
                },
                error:function(result) {
                    res.core.deploy.send({context:context, event:"deploy.advertise.getDeployDetail.failed", data:result});
                }
                });
            break;
        case "deploy.advertise.saveDeployDetail":
            res.service.deploy.saveDeployDetail(message.data, {
                success:function(result) {
                    res.core.deploy.send({context:context, event:"deploy.advertise.saveDeployDetail.successful", data:result});
                },
                error:function(result) {
                    res.core.deploy.send({context:context, event:"deploy.advertise.saveDeployDetail.failed", data:result});
                }
            });
            break;
        case "deploy.advertise.viewTableStore":
            res.service.deploy.viewTableStore(message.data,{
                success:function(result){
                    res.core.deploy.send({context:context, event:"deploy.advertise.viewTableStore.successful", data:result});
                },
                error:function(result){
                    res.core.deploy.send({context:context, event:"deploy.advertise.viewTableStore.failed", data:{}});
                }
            });
            break;
        case "deploy.advertise.editTableStore":
            res.service.deploy.editTableStore(message.data, {
                success:function(result){
                    res.core.deploy.send({context:context, event:"deploy.advertise.editTableStore.successful", data:result});
                },
                error:function(result){
                    res.core.deploy.send({context:context, event:"deploy.advertise.editTableStore.failed", data:result});
                }
            });
            break;
        case "deploy.advertise.viewTableGroup":
            res.service.deploy.viewTableGroup(message.data,{
                success:function(result){
                    res.core.deploy.send({context:context, event:"deploy.advertise.viewTableGroup.successful", data:result});
                },
                error:function(result){
                    res.core.deploy.send({context:context, event:"deploy.advertise.viewTableGroup.failed", data:result});
                }
            });
            break;
        case "deploy.advertise.editTableGroup":
            res.service.deploy.editTableGroup(message.data,{
                success:function(result){
                    res.core.deploy.send({context:context, event:"deploy.advertise.editTableGroup.successful", data:result});
                },
                error:function(result){
                    res.core.deploy.send({context:context, event:"deploy.advertise.editTableGroup.failed", data:result});
                }
            });
            break;
        case "deploy.advertise.getSysFileTree":
            res.service.deploy.getSysFileTree(message.data, {
                success: function(result) {
                    res.core.deploy.send({context:context, event:"deploy.advertise.getSysFileTree.successful", data:result});
                },
                error: function(result) {
                    res.core.deploy.send({context:context, event:"deploy.advertise.getSysFileTree.failed", data:result});
                }
            });
            break;
        case "deploy.advertise.invokeTransfer":
            res.service.deploy.invokeTransfer(message.data, {
                success:function(result) {
                    res.core.deploy.send({context:context, event:"deploy.advertise.invokeTransfer.successful", data:result});
                },
                error:function(result) {
                    res.core.deploy.send({context:context, event:"deploy.advertise.invokeTransfer.failed", data:result});
                }
            });
            break;
        }
