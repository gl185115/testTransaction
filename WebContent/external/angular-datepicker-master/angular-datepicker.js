...
(function withAngular(angular) {
  ...
  .directive('datepicker', ['$window', '$compile', '$locale', '$filter', '$timeout',function manageDirective($window, $compile, $locale, $filter,$timeout) {
    ...
    return {
      ...
      'link': function linkingFunction($scope, element, attr) {
        ...
        var selector = attr.selector
          ...
          , theCalendar
          , defaultPrevButton = '<b class="datepicker-default-button">&laquo;</b>'
          , defaultNextButton = '<b class="datepicker-default-button">&raquo;</b>'
        ...
        $scope.initCanlendar=function initCal(){
        	// now date
        	var nowDate = new Date();
            if (attr.dateFormat) {
            	nowDate = $filter('date')(nowDate, dateFormat);
            }
        	var dateValue=thisInput.context.value?thisInput.context.value:nowDate;
        	if(dateValue){
        		var year = parseInt(dateValue.slice(0, 4), 10);
                var month = parseInt(dateValue.slice(4, 6), 10);
                var day = parseInt(dateValue.slice(6, 8), 10);
                $scope.monthNumber=Number(month);
                //set next month
                $scope.month = $filter('date')(new Date($scope.year + '/' + $scope.monthNumber + '/01'), 'MMMM');
                $scope.year = Number(year);
                $scope.setDaysInMonth($scope.monthNumber, $scope.year);
