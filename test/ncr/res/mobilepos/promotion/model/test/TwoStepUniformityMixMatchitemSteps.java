...
import ncr.res.mobilepos.promotion.model.FirstStepUniformityMixMatchItem;
...
import ncr.res.mobilepos.promotion.model.SecondStepUniformityMixMatchItem;
...
/**
 * The Class TwoStepUniformityMixMatchitemSteps.
 */
public class TwoStepUniformityMixMatchitemSteps extends Steps {

    /** The first step rep. */
    SecondStepUniformityMixMatchItem secondStepUniformity = null;
    ...
    public final void givenMMData(ExamplesTable exampleTables) {
        for (Map<String, String> mmDataTemp : exampleTables.getRows()) {
            ...
            int MMP2 = Integer.parseInt(mmDataTemp.get("MMP2"));
            mmData.setCode(MMCode);
            mmData.setQuantity(new int[] { MMQ, MMQ2 });
            mmData.setDiscountprice(new long[] { MMP, MMP2 });
            mmData.setName(mmDataTemp.get("MMDescription"));
            mmData.setType(MixMatchItem.SECOND_STEP_UNIFORMITY_PRICE);
            secondStepUniformity = new SecondStepUniformityMixMatchItem(mmData.getCode());
            secondStepUniformity.setMixMatchData(mmData);
        }
    }
    ...
    public final void addSale(ExamplesTable exampleTables) {
        for (Map<String, String> data : exampleTables.getRows()) {
            ...
            secondStepUniformity.addItem(sale);
        }
    }
    ...
    public final void computeFirstStepRep() {
        discounts = secondStepUniformity.computeDiscount();
    }
