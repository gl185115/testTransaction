...
public class ListItemsResourceSteps extends Steps {
    ...
    public final void anItemResource() throws SecurityException, NoSuchFieldException, IllegalArgumentException, IllegalAccessException {
    ...
    @When("I get the list of Items with storeid {$storeid} and key {$key} and deviceid {$deviceid} and name {$name}")
    public final void getListItems(final String storeid, final String key,
            final String deviceid, final String name) {
        String keyTemp = key.equalsIgnoreCase("null") ? null : key;
        String nameTemp = name.equalsIgnoreCase("null") ? null : name;
        
        actualSearchedProducts = itemResource.list(storeid, keyTemp,
                deviceid,0,nameTemp);
    }
        ...
        if (data.equals("items")) {
            ...
            try {
	            for (Map<String, String> expItem : expItemRows) {
	                ...
	                Assert.assertEquals(
	                        "Compare the DISPLAYPLU row" + index + ": ",
	                ...
	                Assert.assertEquals("Compare the MDINTERNAL row" + index + ": ",
	                        expItem.get("mdinternal"), actualDataRows.getValue(index, "MDINTERNAL"));
	                Assert.assertEquals("Compare the MDNAME row" + index + ": ", 
	                		String.valueOf(expItem.get("mdname")), 
	                		String.valueOf(actualDataRows.getValue(index, "MDNAME")));
	                Assert.assertEquals("Compare the MDNAMELOCAL row" + index + ": ", 
	                		String.valueOf(expItem.get("mdnamelocal")), 
	                		String.valueOf(actualDataRows.getValue(index, "MDNAMELOCAL")));
	                Assert.assertEquals("Compare the InheritFlag row" + index + ": ", 
	                		String.valueOf(expItem.get("inheritflag")), 
	                		String.valueOf(actualDataRows.getValue(index, "INHERITFLAG")));
	                Assert.assertEquals("Compare the POSMDTYPE row" + index + ": ", 
	                		String.valueOf(expItem.get("nonsales")), 
	                		String.valueOf(actualDataRows.getValue(index, "POSMDTYPE")));
	                Assert.assertEquals("Compare the SUBINT10 row" + index + ": ", 
	                		String.valueOf(expItem.get("subint10")), 
	                		String.valueOf(actualDataRows.getValue(index, "SUBINT10")));
            ...
            for (Map<String, String> expItem : expItemRows) {
                ...
                Assert.assertEquals("Compare the DptNameLocal row" + index + ": ",
                        String.valueOf(expItem.get("dptnamelocal")), String
                                .valueOf(actualDataRows.getValue(index,
                                        "DPTNAMELOCAL")));
    ...
    public final void theListOfItemsAre(final ExamplesTable expectedItems){
    	...
    	Assert.assertEquals("Compare the exact number of Items retrieved"
    			+ " during Item List", expectedItems.getRowCount(),
    			actualItems.size());

    	int i = 0;
        for(Map<String, String> expecedItem : expectedItems.getRows()){
            assertThat("Compare the Items PluCode at row " + i, actualItems.get(i).getItemId(),
            ...
            assertThat("Compare the Items Department at row " + i, actualItems.get(i).getDepartment(),
                    is(equalTo(expecedItem.get("Department"))));
            assertThat("Compare the Items Class at row " + i, actualItems.get(i).getItemClass(),
            ...
            assertThat("Compare the Items Discount at row " + i, "" + actualItems.get(i).getDiscount(),
            ...
            assertThat("Compare the Items InheritFlag at row " + i, actualItems.get(i).getInheritFlag(),
                    is(equalTo(expecedItem.get("InheritFlag"))));
            assertThat("Compare the Items NonSales at row " + i, actualItems.get(i).getNonSales()+"",
                    is(equalTo(expecedItem.get("NonSales"))));
            assertThat("Compare the Items SubInt10 at row " + i, actualItems.get(i).getSubInt10()+"",
