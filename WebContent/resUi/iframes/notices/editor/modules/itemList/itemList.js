...
res.ui.controller("itemList", ["$scope", "$rootScope", "$timeout", function($scope, $rootScope, $timeout){
	...
	$scope.$watch(
		...
		function(newValue, oldValue) {
			if (res.ui.root.context == "Notice" && $rootScope.model.editor.indexEdit == "itemList") {
				res.ui.send({
...
//				res.ui.send({context:"Notice", event: "schedule.get", data: {} });
//				res.ui.send({context:"Notice", event:"file.list", data:{folder:"notices"}});
//				$timeout(function(){ scroll.refresh(); }, 200);
//			}
//		);
	...
	$scope.select = function(index) {
		...
		$rootScope.model.editor.selectedFile = $rootScope.model.editor.files[$scope.indexFile];
	};
}]);