...
public class SetQueueBusterLinkSteps extends Steps {
    ...
    public final void entriesQueueBusterLinkDB() throws Exception{
        dbInit =  new DBInitiator("Mst_PrinterInfo",
                "test/ncr/res/mobilepos/deviceinfo/"
                + "datasets/PRM_QUEUEBUSTER_LINK.xml", DATABASE.RESMaster);
    }
    ...
    @When ("I set queuebuster link of storeid{$storeid}"
            + " and deviceid{$deviceid} to {$queuebusterlink}")
    public final void setposterminalid(final String storeid,
            final String deviceid, String queuebusterlink)
    {
        if(pdc == null){
            pdc = new DeviceInfoResource();
        }
        
        if (queuebusterlink.equalsIgnoreCase("null")) {
            queuebusterlink = null;
        }
        
        resBase = pdc.setQueueBusterLink(
                storeid, deviceid, queuebusterlink);
    }
    ...
    public final void theMST_QUEUEBUSTER_LINKDatabaseTableShouldHaveTheFollowingRow(ExamplesTable expectedDeviceInfoTable) {
        try {
            ...
            for(Map<String, String> expectedDeviceInfo : expectedDeviceInfoTable.getRows()) {
				...
				Assert.assertEquals(
						"Compare the linkqueuebuster in MST_DEVICEINFO row" + i,
						expectedDeviceInfo.get("linkqueuebuster").equalsIgnoreCase("null") ? null
								: expectedDeviceInfo.get("linkqueuebuster"),
