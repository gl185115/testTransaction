package ncr.res.mobilepos.promotion.resource.steps;
...
public class PromotionResourceTestSteps extends Steps {
    ...
    public void aPromotionService() {
        ...
        try {
            ...
            promotioncontext = testpromotionResource
            .getClass().getDeclaredField("context");
            itemcontext = itemResourceObj.getClass().getDeclaredField("context");
    ...
    public final void theWebAPIStartsUpSytstemConfiguration(String systemConDataSet){
        servletContext = null;
        try
        {
            dbInit.ExecuteOperation(DatabaseOperation.CLEAN_INSERT,
            "test/ncr/res/mobilepos/promotion/datasets/" + systemConDataSet);
            servletContext = Requirements.getMockServletContext();
        } catch (/*RuntimeError*/Exception ex) {
            Assert.fail("Can't Start Up WebStoreServer");
        }
    ...
    @When("a Item Entry at promotion with parameters RetailStoreID{$retailStoreID} WorkStationId{$workStationID} SequenceNo{$seqNo} TransactionJson{$transactionJson}")
    public final void aItemEntryAtPromotionWithParameters(
             String retailStoreId,  String workStationId,  String sequenceNo,  String transaction, String companyId, String transactionJson) {
        ...
        actualResultBase =
            testpromotionResource.itemEntry(retailStoreId, workStationId,
                    sequenceNo, transaction, companyId, transactionJson);
    }
    ...
    public final void itemQuantityAtPromotionWithParameters(final String retailStoreId,
            final String workStationId, final String sequenceNumber, final String transactionJson, final String isDelete) {
        String retailStoreIdTemp = retailStoreId.equalsIgnoreCase("null") ? null: retailStoreId;
        String workStationIdTemp = workStationId.equalsIgnoreCase("null") ? null: workStationId;
        String sequenceNumberTemp = sequenceNumber.equalsIgnoreCase("null") ? null: sequenceNumber;
        String transactionJsonTemp = transactionJson.equalsIgnoreCase("null") ? null: transactionJson;
        String isDeleteTemp = transactionJson.equalsIgnoreCase("null") ? null: isDelete;
        actualResultBase =
            testpromotionResource.itemUpdate(retailStoreIdTemp, workStationIdTemp, sequenceNumberTemp, transactionJsonTemp);
    }
    ...
    public final void itemUpdateAtPromotionWithParameters(final String retailStoreId,
            final String workStationId, final String sequenceNumber, final String transactionJson, final String isDelete) {
        String retailStoreIdTemp = retailStoreId.equalsIgnoreCase("null") ? null: retailStoreId;
        String workStationIdTemp = workStationId.equalsIgnoreCase("null") ? null: workStationId;
        String sequenceNumberTemp = sequenceNumber.equalsIgnoreCase("null") ? null: sequenceNumber;
    ...
    public void iEndTheTransaction(final String retailstoreid, final String workstationid, final String sequencenumber, final String transactionJson) {
        actualResultBase = testpromotionResource.endTransaction(retailstoreid, workstationid, sequencenumber, transactionJson);
    }
	...
	public final void initdatasetsDpt(final String dataset) {
		try {
			dbInit.ExecuteOperation(DatabaseOperation.CLEAN_INSERT,
					"test/ncr/res/mobilepos/promotion/datasets/" + dataset);
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
    ...
    public final void saleDataShouldBe(final ExamplesTable expectedItems) {
        PromotionResponse promotionResponse =
            (PromotionResponse) actualResultBase;
        ...
        assertThat("sale is equal to null",
                actualSale == null,
                is(equalTo(false)));        
        for(Map<String, String> expectedItem : expectedItems.getRows()){
            assertThat("Compare the ItemEntryID at row ", "" + actualSale.getItemEntryId(),
            ...
            assertThat("Compare the Department at row ", actualSale.getDepartment(),
                    is(equalTo(expectedItem.get("Department"))));
            assertThat("Compare the Description EN at row",
                    "" + actualSale.getDescription().getEn(),
            ...
            assertThat("Compare the Discount row ",
            		"" + actualSale.getDiscount(),
                    is(equalTo(expectedItem.get("Discount"))));
            assertThat("Compare the DiscountAmount at row",
                    "" + actualSale.getDiscountAmount(),
                    is(equalTo(expectedItem.get("DiscountAmount"))));
            ...
            assertThat("Compare the Discountable at row",
                    ("" + actualSale.getDiscountable()).toLowerCase(),
                    is(equalTo(expectedItem.get("Discountable").toLowerCase())));
            assertThat("Compare the AgeRestrictedFlag at row",
                    "" + actualSale.getAgeRestrictedFlag(),
                    is(equalTo(expectedItem.get("AgeRestrictedFlag"))));
            assertThat("Compare the CouponFlag at row",
                    "" + actualSale.getCouponFlag(),
                    is(equalTo(expectedItem.get("CouponFlag"))));
        }
    }
    ...
    public final void shouldBeNoPromotionData() {
        ...
        Assert.assertTrue("Promotion data should be empty",
                promotionResult == null 
                || (promotionResult != null && promotionResult.getDiscounts() == null) 
                || (promotionResult != null && promotionResult.getDiscounts().size() == 0));
    }
    ...
    public final void thereShouldBeNoPromotionDataForUpdateQuantity() {
        PromotionResponse promotionResponse =
            (PromotionResponse) actualResultBase;
        Assert.assertTrue("Promotion data should be null", 0 == promotionResponse.getPromotion().getDiscounts().size());
    }
    ...
    public final void theListOfItemsAre(final ExamplesTable expectedItems){
        ...
        assertThat("promotion is equal to null",
                ...
                .getPromotion() == null,
                is(equalTo(false)));
        List<Discount> actualItems =  promotionResponse
                                        .getPromotion().getDiscounts();
        
        if (actualItems == null) {
            Assert.assertTrue("It is expected that there are no discounts", expectedItems.getRowCount() == 0);
            return;
        }
        ...
        for(Map<String, String> expecedItem : expectedItems.getRows()){
            assertThat("Compare the RewardID at row " + i, "" + actualItems.get(i).getRewardID(),
                    is(equalTo(expecedItem.get("RewardID"))));
            assertThat("Compare the EarnedRewardID row " + i, actualItems.get(i).getEarnedRewardID(),
                    is(equalTo(expecedItem.get("EarnedRewardID"))));
            assertThat("Compare the PromotionCode at row " + i,
                    actualItems.get(i).getPromotionCode(),
            ...
            assertThat("Compare the ItemEntryID at row " + i, actualItems.get(i).getItemEntryID(),
    ...
    public final void revokePresent() {
        PromotionResponse promotionResponse =
            (PromotionResponse) actualResultBase;
        assertThat("revoke is equal to null",
                promotionResponse
                .getPromotion().getRevoke() == null,
                is(equalTo(false)));
    }
    ...
    public final void theListOfItemsAreforForUpdateQuantity(final ExamplesTable expectedItems){
        PromotionResponse promotionResponse =
            (PromotionResponse) actualResultBase;
    ...
    public final void setPromotionTermItemsContext(String value){
        ...
        servletContext.setAttribute(GlobalConstant.PROMOTION_TERMINAL_ITEMS, valueTemp);
    }


    @SuppressWarnings("unchecked")
    @Then("the following item quantity for Sale(s) in general promotion mix match with TerminalItem{$termitem}: $expectedSales")
    public final void theFollowingItemQuantityForSaleInGeneralPromotionMixMatch(final String termitem,
            final ExamplesTable expectedSales){
        actualTerminalItems = (Map<String, TerminalItem>)
        servletContext.getAttribute(GlobalConstant.PROMOTION_TERMINAL_ITEMS);

        List<Sale> actualSales = actualTerminalItems.get(termitem)
                    .getMixMatchItem(MixMatchItem.GENERAL_PROMOTION).getItemList();
        Assert.assertEquals("Compare the number of Sales", expectedSales.getRowCount(), actualSales.size());

        int i = 0;
        for (Map<String, String> expectedSale : expectedSales.getRows()) {
            Sale actualSale = actualSales.get(i);
            Assert.assertEquals("Compare ActualSalesUnitPrice for Sale in row" + i,
                    Double.parseDouble(expectedSale.get("ActualSalesUnitPrice")),
                    actualSale.getActualSalesUnitPrice());
            ...
            Assert.assertEquals("Compare ItemID for Sale in row" + i,
                    expectedSale.get("ItemID"),
                    actualSale.getItemId());
            Assert.assertEquals("Compare ItemIDType for Sale in row" + i,
                    expectedSale.get("ItemIDType"),
                    actualSale.getItemIdType());
            ...
            Assert.assertEquals("Compare RegularSalesUnitPrice for Sale in row" + i,
                    Double.parseDouble(expectedSale.get("RegularSalesUnitPrice")),
                    actualSale.getRegularSalesUnitPrice());
            Assert.assertEquals("Compare Discount for Sale in row" + i,
                    expectedSale.get("Discount"),
                    actualSale.getDiscount());
